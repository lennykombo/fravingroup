...
export default function CorporateNavbar() {
  ...
  return (
    <header className="fixed w-full z-[100]">
      {/* Top Bar *//*
      <div className={`bg-brand-navy text-[10px] text-zinc-400 py-2 px-12 hidden lg:flex justify-between items-center transition-all ${scrolled ? '-translate-y-full' : ''}`}>
        <div className="flex gap-6 font-bold uppercase tracking-widest">
           <span className="flex items-center gap-2"><Phone size={10} className="text-brand-orange"/> +254 700 000 000</span>
           <span className="flex items-center gap-2"><Mail size={10} className="text-brand-orange"/> info@fravin.com</span>
        </div>
        <span>Mon - Fri: 8:00 AM - 5:00 PM</span>
      </div>

      {/* Main Nav *//*
      <nav className={`transition-all duration-500 px-6 lg:px-12 py-5 ${scrolled ? "bg-white shadow-xl py-3" : "bg-white/90 backdrop-blur-md"}`}>
        <div className="max-w-7xl mx-auto flex justify-between items-center">
          <Link to="/fravin" className="flex flex-col leading-none">
            <span className="text-xl lg:text-2xl font-black tracking-tighter text-brand-navy">FRAVIN<span className="text-brand-orange">GROUP</span></span>
            <span className="text-[7px] font-bold tracking-[0.3em] text-zinc-400 mt-1 uppercase">Redefine. Deliver. Excel.</span>
          </Link>

          <div className="hidden lg:flex items-center gap-8 text-[11px] font-black uppercase tracking-widest text-zinc-500">
            <Link to="/fravin" className="hover:text-brand-orange transition">Home</Link>
            <Link to="/fravin/about" className="hover:text-brand-orange transition">About Us</Link>
            <div className="flex items-center gap-1 cursor-pointer hover:text-brand-orange transition">Integrated Solutions <ChevronDown size={12}/></div>
            <Link to="/fravin/projects" className="hover:text-brand-orange transition">Projects</Link>
            <button className="bg-brand-orange text-white px-8 py-3 rounded-md font-bold hover:bg-brand-navy transition shadow-lg shadow-orange-500/20">Request a Quote</button>
          </div>

          <button onClick={() => setMobileOpen(true)} className="lg:hidden text-brand-orange"><Menu size={28}/></button>
        </div>
      </nav>
      ...
      <AnimatePresence>
        {mobileOpen && (
          <motion.div initial={{ x: '100%' }} animate={{ x: 0 }} exit={{ x: '100%' }} className="fixed inset-0 bg-brand-navy z-[200] p-8 flex flex-col">
            <div className="flex justify-between items-center mb-12">
              <span className="text-white font-black text-2xl">FRAVIN</span>
            ...
            <div className="flex flex-col gap-8 text-4xl font-black text-white tracking-tighter uppercase">
              <Link to="/fravin" onClick={()=>setMobileOpen(false)}>Home</Link>
              <Link to="/fravin/about" onClick={()=>setMobileOpen(false)}>About</Link>
              <Link to="/fravin/services" onClick={()=>setMobileOpen(false)}>Solutions</Link>
            </div>
            <button className="mt-auto bg-brand-orange text-white py-6 rounded-xl font-black uppercase tracking-widest">Request a Quote</button>
          </motion.div>
        )}
      </AnimatePresence>
    </header>
